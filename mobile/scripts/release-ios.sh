#!/usr/bin/env bash
# Local iOS release helper: preflight checks + EAS build (+ optional submit).
#
# Usage:
#   ./scripts/release-ios.sh <preview|production> [--submit]
#
# See docs/MOBILE-APP-STORE-DEPLOYMENT.md for the full release flow and the
# one-time EAS/Apple credential setup this script assumes is already done.
set -euo pipefail
cd "$(dirname "$0")/.."
...
npx expo config --json 2>/dev/null | node -e "
  ...
  process.stdin.on('end', () => {
    ...
    console.log('  version=' + cfg.version + '  ios.buildNumber=' + (cfg.ios && cfg.ios.buildNumber) + '  bundleIdentifier=' + (cfg.ios && cfg.ios.bundleIdentifier));
  });
"
...
BUILD_ARGS=(build --platform ios --profile "$PROFILE" --non-interactive)
if $SUBMIT; then
  BUILD_ARGS+=(--auto-submit)
  echo "==> npx eas-cli ${BUILD_ARGS[*]}  (will build, then submit to App Store Connect on success)"
else
...
if $SUBMIT; then
  echo "==> Next: App Store Connect → attach the new build to a version → fill What's New → Submit for Review."
fi
