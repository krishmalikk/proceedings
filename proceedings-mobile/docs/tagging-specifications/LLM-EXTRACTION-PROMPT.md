...
Only the APPLICANT (the person writing the post / whose immigration case it is)
and, if applicable, their LEGAL SPOUSE are subjects of the tagging. Information
about friends, boyfriends, girlfriends, parents, siblings, colleagues, or other
third parties is BACKGROUND ONLY and MUST NOT populate per-applicant fields:
...
- Do NOT set `resident_of_country`, `citizen_of_country`, `born_in_country`,
  or `travel_country` based on a friend / boyfriend / family member's facts.
...
- Process tags the post never mentions: do NOT add `emergency-visa-appointment`,
  ...
  - `change-status-options`: requires the APPLICANT to be (planning to be) in the
    ...
    contemplated.
  - `visa-refused`: requires an ACTUAL refusal (including a 221(g) refusal). Do
    NOT emit when the visa was issued/approved.
