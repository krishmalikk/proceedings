...
- `APP_SOURCE_SYSTEM`, `APP_BASE_URL` — the website's **provenance identity** for first-party postings (D-055). Default `usajourney` / `https://usajourney.ai` (the registered domain); override per-env with `APP_SOURCE_SYSTEM=<name>`, `APP_BASE_URL=https://<domain>` — a config flip, no code change. The `channel` token stays `"app"` (controlled vocabulary — the domain never goes there).
